...
def theta_calculator(peak_pos, peak_row_values, peak_col_values, e_time, fov):
    ...
    """
    principal_distance = tr.compute_radial_distance(
        peak_pos, e_time
...
def save_cross_section_plot(
    ...
    y_r = [
        ...
        * mean(r_values[int(len(r_values) / 2) - 3 : int(len(r_values) / 2) + 3])
        for theta in theta_r
    ]  # Define the ground truth model for the main row as the cos^3(theta) * the max value of the radiance (outliers excluded)
    y_c = [
        pow(cos(radians(theta)), 3)
        * mean(c_values[int(len(c_values) / 2) - 3 : int(len(c_values) / 2) + 3])
...
def cross_section_tester(images, tot_img, exp_time, fov, output_path):
    ...
    theta_row, theta_col, row_distance, col_distance = theta_calculator(
        peak_pos=tr.extract_center_peak(images)[0][0],
...
def save_distance_plot(dist, radiance, output, ext=".svg", annotation=False):
    ...
    x, y = ut.generate_quadratic_model(
        min(dist), max(dist), max(radiance), 1000
...
def compute_norm_factor(tot_img, o_img_path, out_file=None):
    ...
    norm_factor = round(
        float(mean([mean_r, mean_g, mean_b])), 3
    )  # Compute the norm factor as the overall mena

    print(
        "The normalization factor is: %.3f \n" % norm_factor
    )  # Print the overall mean as the normalization factor
...
def plot_norm_factor(folder_path, rgb_path, out_path):
    ...
    for i, j in zip(samples, expected):  # Add the annotations on the point
        if j != expected[-1]:
            plt.annotate(
                str(format_float_scientific(j, precision=1, exp_digits=1, trim="-")),
                xy=(i + 3000, j),
            )
        else:
            plt.annotate(
    ...
    plt.xticks(
        range(min(samples), max(samples) + 10000, 15000),
        labels=[
            format_float_scientific(val, precision=1, exp_digits=1, trim="-")
            for val in range(min(samples), max(samples) + 10000, 15000)
        ],
