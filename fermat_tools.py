...
from modules.transient_handler import (
    extract_peak,
    active_beans_percentage,
    clear_tr_ratio,
    clear_tr_energy,
)
...
def prepare_fermat_data(
    ...
    rt_depthmap = roto_transl(
        coordinates_matrix=copy(depthmap)
    ...
    if data_clean:
        for i in range(data.shape[0]):
            data[i, :, :] = tr.clean_transient_tail(
...
def np2mat(
    ...
    data_grid_size = array(
        data_grid_size, dtype=float64
    )  # Convert the data_grid_size from a list to a ndarray
    ...
    io.savemat(
        str(file_path),
        mdict={
            "detGridSize": np_flip(data_grid_size),
            "detLocs": det_locs,
            "srcLoc": src_loc,
...
def undistort_depthmap(dph, dm, k_ideal, k_real, d_real):
    ...
    if dm == "FISHEYE":
        [map1, map2] = fisheye.initUndistortRectifyMap(
            k_real, d_real, eye(3), k_ideal, shape_depth, CV_16SC2
        )
    ...
    depth[mask_valid_positive] = divide(
        depth[mask_valid_positive], mask_valid[mask_valid_positive]
    )

    z_matrix = depth
    x_matrix = (tile(arange(z_matrix.shape[1]), (z_matrix.shape[0], 1))).astype(
        dtype=float
    )  # [[0,1,2,3,...],[0,1,2,3,..],...]
    y_matrix = tile(arange(z_matrix.shape[0]), (z_matrix.shape[1], 1)).T.astype(
        dtype=float
    )  # [....,[1,1,1,1, ...][0,0,0,0,...]]
...
def compute_los_points_coordinates(
    images: ndarray, mask: ndarray, k_matrix: ndarray, channel: int, exp_time: float
) -> ndarray:
    """
    ...
    for i in range(mask.shape[0]):
        for j in range(mask.shape[1]):
            if mask[i, j] != 0:
                dph[i, j] = tr.compute_radial_distance(
                    peak_pos=tr.extract_peak(images[:, i, j, :])[0][channel],
...
def coordinates_matrix_reshape(data: ndarray, mask: ndarray) -> ndarray:
    ...
    )  # Create the final matrix (full of zeros)
    m_index = 0
...
def reshape_fermat_transient(
    transient: ndarray, grid_shape: tuple, flip_x: bool = False, flip_y: bool = False
) -> ndarray:
    """
    ...
    if flip_x:
        for i in range(0, transient.shape[0], grid_shape[1]):
            transient[i : (i + grid_shape[1]), :] = flip(
                transient[i : (i + grid_shape[1]), :], axis=0
            )  # If flip_x is True revers the order of the data in the x coordinate

    reshaped_transient = zeros(
        transient.shape
    )  # Initialize a ndarray full of zeros of the same size of the input transient one
    ...
    if flip_y:
        for i in range(0, reshaped_transient.shape[0], grid_shape[0]):
            reshaped_transient[i : (i + grid_shape[0]), :] = flip(
...
def rmv_first_reflection_fermat_transient(
    transient: ndarray, file_path: Path = None, store: bool = False
) -> ndarray:
    """
    ...
    if not mono:
        peaks = [
            nanargmax(transient[:, :, channel_i], axis=1)
    ...
    if not mono:
        for channel_i in tqdm(range(transient.shape[2])):
            for pixel in range(transient.shape[0]):
                ...
                valid_zero_indexes = zeros_pos[
                    where(zeros_pos > peaks[channel_i][pixel])
                ]
        ...
        for pixel in range(transient.shape[0]):
            if glb_images[pixel, 0] != -2:
                shifted_transient = delete(
                    glb_images[pixel, :], [where(glb_images[pixel, :] == -1)]
                )
                glb_images_shifted[
    ...
    if file_path and store:
        ...
        )  # Save the loaded images as a numpy array
    return glb_images_shifted


def rmv_sparse_fermat_transient(
    transients: ndarray, channel: int, threshold: int, remove_data: bool
) -> ndarray:
    """
    ...
    for i in range(transients.shape[0]):  # Analyze each transient one by one
        ...
        zeros_pos = where(transients[i, :, channel] == 0)[
            0
        ]  # Find where all the zeros are located
        first_zero_after_peak = zeros_pos[where(zeros_pos > peak)][
            0
        ]  # Keep only the first zero after the direct component
        if (
            active_beans_percentage(transients[i, first_zero_after_peak:, channel])
            < threshold
...
def roto_transl(coordinates_matrix: ndarray) -> ndarray:
    ...
    next_x_pos = nonzero(coordinates_matrix[nearest_center_pos[0], :, 0])[
        0
    ...
    next_y_pos = nonzero(coordinates_matrix[:, nearest_center_pos[1], 0])[
        0
    ...
    normal = cross(
        subtract(next_x_coord, nearest_center_coord),
        subtract(next_y_coord, nearest_center_coord),
    ...
    if n_x != 0 or n_y != 0:
        ...
        o_rot_matrix = array(
            [[0, 1, 0], [-1, 0, 0], [0, 0, 1]], dtype=float32
        ...
        rot_matrix = array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=float32)

    tr_vector = matmul(
        rot_matrix, reshape(nearest_center_coord, [3, 1]), dtype=float32
    )  # Define a translation vector that aims to center the plane on the nearest_center point

    # Build the final roto-transl matrix
    rototransl_matrix = concatenate(
        (rot_matrix, negative(tr_vector)), axis=1
    )  # Add to the left of the rotation matrix the translation one
    rototransl_matrix = concatenate(
        (rototransl_matrix, array([[0, 0, 0, 1]], dtype=float32)), axis=0
    )  # Add to the bottom the vector [0 0 0 1]

    non_zero_pos = nonzero(
    ...
    for r in unique(non_zero_pos[0]):
        for c in unique(non_zero_pos[1]):
            coord_vector = concatenate(
                (
                    reshape(coordinates_matrix[r, c, :], (3, 1)),
                    array([[1]], dtype=float32),
                ),
                axis=0,
            )  # For every non-zero point of the coordinates' matrix concatenate a 1 at the end and reshape it from [3], to [4, 1]
            coordinates_matrix[r, c, :] = matmul(
                rototransl_matrix, coord_vector, dtype=float32
            )[
                :-1, 0
            ]  # Apply the roto-translation to each non-zero point and remove the final 1
    coordinates_matrix[
        where(coordinates_matrix == -0.0)
