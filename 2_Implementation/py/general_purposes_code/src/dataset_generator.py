...
def arg_parser(argv):
    ...
    arg_help = "{0} -p <perm> -d <dataset> -b <batch> -t <template> -s <seed> -i <img> -g <grid>".format(argv[0])  # Help string
    ...
    return [arg_permutation_list_path, arg_dataset_file_path, arg_batches_path, arg_template_path, arg_rnd_seed, arg_img_shape, arg_grid]


if __name__ == '__main__':
    perm_list_path, dts_file_path, batches_path, template_path, rnd_seed, img_shape, pattern = arg_parser(sys.argv)  # Recover the input and output folder from the console args

    # CONSTANTS #
    N_CLASSES = 4                                        # Number of classes
    N_ELM_PER_CLASS = 336                                # Number of elements per class (must be divisible by 2, 7 and 8)
    ...
    ROUGHNESS = tuple(i/100 for i in range(30, 101, 5))  # Roughness of the front wall material
    OBJ_NAMES = ("Cube", "Cone", "Cylinder",
                 "Parallelepiped", "Sphere",
                 "Concave plane", "Random")              # List that contains the name of the different objects
    ...
    obj_tr_list = [[i / 10 for i in range(0, 4)],
    ...
    obj_full_rot_list = [[i for i in range(-90, 91)],
    ...
    obj_partial_rot_list = [[i for i in range(-90, 91)],
                            [i for i in range(-90, 91)],
                            [0]]
    # List that contains all the possible rotations that is granted to the camera
    # [[possible x rotations], [possible y rotations], [possible z rotations]]
    cam_rot_list = [[i for i in range(85, 96)],
    ...
    cam_pos_list = [[i / 10 for i in range(10, 14)],
