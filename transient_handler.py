...
def img_matrix(channels, verbose=False):
    ...
    if not mono:
        ...
        if verbose:
            ...
            sleep(0.02)
        images = empty(
            [shape(channels)[2], shape(channels)[0], shape(channels)[1]], dtype=float32
        )  # Empty array that will contain all the images
        # Fuse the channels together to obtain a proper [A, R, G, B] image
...
def total_img(images, out_path=None, n_samples=None):
    ...
    if n_samples is not None:
        ...
        total_image = divide(summed_images, normalization_factor).astype(
            float32
        )  # 17290
    else:
...
def extract_peak(transient: ndarray) -> tuple:
    ...
    if not mono:
        ...
        for i in range(transient.shape[1]):
            if m_isnan(nanmin(transient[:, i])) and m_isnan(
                nanmax(transient[:, i])
                ...
                peak_pos.append(
                    nanargmax(transient[:, i], axis=0)
                )  # Extract the position of the maximum value in the provided transient
        peak_values = [
            transient[peak_pos[i], i] for i in range(transient.shape[1])
        ]  # Extract the radiance value in the peak position
    else:
        if m_isnan(nanmin(transient)) and m_isnan(
            nanmax(transient)
        ):  # Check for all nan case
            peak_pos = (
...
def extract_center_peak(images):
    ...
    try:
        max_index_r, max_index_g, max_index_b = [
            nanargmax(images[:, :, :, i], axis=0) for i in range(images.shape[3])
        ...
        max_index_r, max_index_g, max_index_b = [
            nanargmax(channel, axis=2) for channel in channels_no_nan
        ]
    peak_pos = [
        data[int(data.shape[0] / 2), int(data.shape[1] / 2)]
        for data in [max_index_r, max_index_g, max_index_b]
    ]  # Extract the position of the maximum value in the middle pixel
    peak_values = [
        images[peak_pos[i], int(images.shape[1] / 2), int(images.shape[2] / 2), i]
        for i in range(images.shape[3])
...
def rmv_first_reflection_transient(
    ...
    if not mono:
        peaks = [
            nanargmax(transient[:, channel_i], axis=0)
...
def rmv_first_reflection_img(images, file_path=None, store=False, verbose=True):
    ...
    if not mono:
        peaks = [
            nanargmax(images[:, :, :, channel_i], axis=0)
    ...
    if not mono:
            ...
            for pixel_r in range(images.shape[1]):
                for pixel_c in range(images.shape[2]):
                    ...
                    valid_zero_indexes = zeros_pos[
                        where(zeros_pos > peaks[channel_i][pixel_r, pixel_c])
                    ]
                    if valid_zero_indexes.size == 0:
        ...
        for pixel_r in trange(images.shape[1], leave=False):
            for pixel_c in range(images.shape[2]):
                ...
                valid_zero_indexes = zeros_pos[
                    where(zeros_pos > peaks[pixel_r, pixel_c])
                ]
                if valid_zero_indexes.size == 0:
...
def histo_plt(
    ...
    if interval is not None:
        ...
        try:
            if not mono:
                plt_start_pos = [
                    where(radiance[:, channel] != 0)[0][0] - 10
                    for channel in range(0, 3)
                ]
                plt_end_pos = [
                    where(radiance[:, channel] != 0)[0][-1] + 11
                    for channel in range(0, 3)
                ]
            else:
...
def rmv_glb(transient: ndarray) -> ndarray:
    ...
    if not mono:
        peaks = [
            nanargmax(transient[:, channel_i], axis=0)
            for channel_i in range(transient.shape[1])
        ]  # Find the index of the maximum value in the third dimension
    else:
        peaks = nanargmax(
            transient, axis=0
    ...
    if not mono:  # If the images are RGBA
        for channel_i in range(transient.shape[1]):  # For each channel
            zeros_pos = where(transient[:, channel_i] == 0)[
                0
            ]  # Find the index of the zeros
            valid_zero_indexes = zeros_pos[
                where(zeros_pos > peaks[channel_i])
...
def clear_tr_ratio(transient: ndarray, method: str = "otsu") -> ndarray:
    ...
    ratio = zeros(
        tr.shape[0]
    )  # Create the ndarray that will contain the ratio data as an array full of zeros
    for i in range(tr.shape[0]):
        ratio[i] = direct_global_ratio(tr[i, :, 1])[
            0
        ]  # Populate the ratio ndarray computing the ratio value for each transient
...
def clear_tr_energy(transient: ndarray, threshold: int) -> ndarray:
    ...
    glb = zeros(
        transient.shape
    )  # Create the ndarray that will contain the global component as an array full of zeros
    glb_sum = zeros(
        transient.shape[0]
    )  # Create the ndarray that will contain the sum of the global component as an array full of zeros
    ...
    for i in trange(transient.shape[0], desc="cleaning transient"):
        ...
        max_glb_value = max(
            max_glb_value, np_max(glb_sum[i])
...
def compute_distance_map(tr: ndarray, fov: int, exp_time: float) -> ndarray:
    ...
    d_map = zeros(
        [tr.shape[1], tr.shape[2]]
    )  # Create the ndarray that will contain the distance map as an array full of zeros
    if tr.shape[3] == 4:  # If the image is RGBA
        non_zero = where(
            np_sum(tr[:, :, :, -1], axis=0) != 0
