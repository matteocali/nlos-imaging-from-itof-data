...
class PredictiveModel:
    def __init__(self, name, dim_b, freqs, P, saves_path, use_S1=True, use_data=True, use_transient=True, dim_t=2000,
        ...
        self.use_S1 = use_S1  # whether to use the S1 dataset
        self.use_data = use_data  # whether to train on transient data
        self.use_transient = use_transient  # if we want to use the transient data
        ...
        self.Autoencoder = Autoencoder_Interp.Autoencoder_Interp(self.dim_b, self.dim_t, self.dim_encoding,
                                                                 self.fil_encoder)
    ...
    def load_weights(self, weight_filename_d=None, weight_filename_v=None, weight_filename_z=None):
        ...
        if weight_filename_z is not None:
            self.TransientNet.load_weights(weight_filename_z, by_name=True)
    ...
    def save_weights(self, suffix):
        ...
        weight_filename_pred_v_enc = self.name + '_pred_v_enc_' + suffix + '.h5'
        ...
        self.predv_encoding.save_weights(os.path.join(self.checkpoint_path, weight_filename_pred_v_enc))
    ...
    def def_DirectCNN(self):
        ...
        phi = tf.where(tf.math.is_nan(phi), 0.,
                       phi)  # Needed for the first epochs to correct the output in case of a 0/0
    ...
    def loss_walls(self, data_dict, epoch):
        ...
        if self.use_transient:
            ...
            ratio_gd = sum_xg / (
                        sum_x - sum_xg)  # Ratio between global and direct components. Used to give weights to the EMD function
    ...
    # Training loop
    def training_loop(self, train_S1_loader=0, test_S1_loader=0, train_w_loader=0, test_w_loader=0, val_S3_loader=0,
                      final_epochs=2000, init_epoch=0, print_freq=5, save_freq=5, pretrain_filenamed=None,
                      pretrain_filenamev=None, pretrain_filenamez=None, use_S1=True, use_walls=True):
        ...
        if (not self.use_S1) and (not self.use_data):
        ...
        if not self.use_transient:
            ...
            loss_test = loss_list_testw[0][
                4]  # If we are using the transient, we want to optimize on the transient prediction, not S3
        ...
        for epoch in range(init_epoch, final_epochs):
                ...
                if self.use_data:
                    ...
                    if self.use_transient:
                        ...
                        if grads_predv_enc[0] is not None:
                            self.optimizer.apply_gradients(
                                grads_and_vars=zip(grads_predv_enc, self.predv_encoding.trainable_variables))
            ...
            if self.use_data:
                loss_testw, loss_list_testw = self.loss_perbatches(test_w_loader, N_batches=test_w_loader.N_batches,
                                                                   epoch=epoch)
            if not self.use_transient:
                loss_testS3, loss_list_testS3 = self.loss_perbatches(val_S3_loader, N_batches=val_S3_loader.N_batches,
                                                                     epoch=epoch)
            ...
            if loss_test < self.best_loss_test:
                ...
                old_weight_filename_predv_enc = self.name + '_predv_enc_e' + str(self.best_epoch) + '_best_weights.h5'
