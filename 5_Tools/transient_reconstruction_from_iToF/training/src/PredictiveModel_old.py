...
class PredictiveModel:
    ...
    def __init__(self, name, dim_b, freqs, P, saves_path, use_S1=True, use_walls=True, use_transient=True, dim_t=2000, fil_size=128,fil_denoise_size=32,fil_z_size=32):
    ...
    def def_DirectCNN(self):
        ...
        v_res = layers.Conv2D(filters=self.fn2,
                           kernel_size=1,
                           strides=1,
                           padding="valid",
                           data_format='channels_last',
        ...
        model_pred = tf.keras.Model(inputs=v_in, outputs=[v_out_g,v_out_d,phi], name=self.name)
        return model_pred
    ...
    def def_TransientNet(self):
        ...
        phid = tf.math.atan2(v_in_d[...,0],v_in_d[...,self.fn])
        phid = (phid+2*math.pi)%(2*math.pi)
        posd = phid*utils.c()/(self.freqs[0]*4*math.pi)
        posd = posd/utils.max_d()  # put between 0 and 1
        posd = tf.expand_dims(posd,axis=-1)
    ...
    def def_loss(self, data_dict,  epoch):
        ...
        if data_dict["name"]  == 1:
            loss, loss_list = self.loss_walls(data_dict,epoch)
    ...
    def loss_walls(self, data_dict, epoch):
        ...
        v_out_g,v_out_d,phi_pr = self.DirectCNN(v_nf)
        # If desired, compute also the transient prediction
        if self.use_transient:
            z_pr = self.TransientNet([v_out_g,v_out_d])
            x_pr = self.gen_model(z_pr)
            x_pr = tf.squeeze(x_pr)
            #loss_emd = self.EMDc_loss(x_g,x_pr,ratio_gd) 
            loss_emd = self.EMDc_loss(x_g,x_pr) 
        else: 
            loss_emd = 0
        # Compute the loss between ground truth and predicted phase displacement
        ...
        # Compute the loss between true and predicted direct measurements
        v_d = v_d[:,i_mid,i_mid,:]
        v_out_d = tf.squeeze(v_out_d)
        loss_vd = tf.math.abs(v_out_d-v_d)
        loss_vd = tf.math.reduce_mean(loss_vd)
        
        
        # Total loss
        loss_value =   loss_vd+loss_emd 

        # Keep track of some losses
        losses = [loss_vd,loss_phi,loss_emd]
        loss_list = [losses]
    ...
    def training_loop(self, train_S1_loader=0, test_S1_loader=0, train_w_loader=0, test_w_loader=0, val_S3_loader=0, final_epochs=2000, init_epoch=0, print_freq=5, save_freq=5, pretrain_filenamed=None,pretrain_filenamev=None,pretrain_filenamez=None,use_S1=True,use_walls=True):
        ...
        if self.use_S1:
            ...
            loss_testS1, loss_list_testS1 = self.loss_perbatches(test_S1_loader,  N_batches=1)
        else:
            loss_trainS1 = 0
            loss_testS1 = 0
        if self.use_walls:
            loss_trainw, loss_list_trainw = self.loss_perbatches(train_w_loader, N_batches=1)
            loss_testw, loss_list_testw = self.loss_perbatches(test_w_loader,  N_batches=1)
        else:
            loss_trainw = 0
            loss_testw = 0
        if not self.use_transient:
            loss_testS3, loss_list_testS3 = self.loss_perbatches(val_S3_loader, N_batches=1)
        ...
        with summary_tr.as_default():
            ...
            if self.use_walls:
                ...
                tf.summary.scalar('err_phi_w', loss_list_trainw[0][1], step=init_epoch)
        ...
        with summary_val.as_default():
            ...
            if self.use_walls:
                ...
                tf.summary.scalar('err_phi_w', loss_list_testw[0][1], step=init_epoch)
        ...
        if not self.use_transient:
            loss_test = loss_testS3
        else:
            loss_test = loss_list_testw[0][2]  # If we are using the transient, we want to optimize on the transient prediction, not S3
        ...
        for epoch in range(init_epoch, final_epochs):
            ...
            while True:
                ...
                if self.use_walls:
                    with tf.GradientTape() as denoise_tape2, tf.GradientTape() as predv_tape2, tf.GradientTape() as predz_tape:
            ...
            with summary_tr.as_default(): 
                ...
                if self.use_walls:
                    ...
                    tf.summary.scalar('err_phi_w', loss_list_trainw[0][1], step=epoch+1)
            ...
            with summary_val.as_default(): 
                ...
                if self.use_walls:
                    ...
                    tf.summary.scalar('err_phi_w', loss_list_testw[0][1], step=epoch+1)
