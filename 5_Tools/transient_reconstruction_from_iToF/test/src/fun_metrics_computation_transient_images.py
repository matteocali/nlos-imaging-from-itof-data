...
def metrics_computation_transient_images(trans, trans_nod, pred_trans_nod, pred_vd, v, freqs=np.array((20e06,50e06,60e06),dtype=np.float32), fl_barragan=False, mask=None, fl_matte=False):
    ...
    if mask is None:
        ...
        MAE_depth_base = np.nansum(mask*np.abs(gt_pos-pred_base[-1]))/np.nansum(mask) # Compute against prediction at 60 MHz
    ...
    if fl_plots:
        ...
        #posy = [410,270,70,380]
        #posx = [350,366,366,110]
        posy = [240, 250, 250, 110]
        posx = [230, 120, 70, 200]
        #posx = [150,165,150,100]
        #posy = [260,220,175,280]
        xsteps = np.arange(0,5,5/2000)
        for ix,iy in zip(posx,posy):
            plt.figure()
            plt.plot(xsteps,trans_nod[ix,iy,:],"g")
            plt.plot(xsteps,pred_trans_nod[ix,iy,:],"b")
            plt.xlabel("Depth [m]")
            plt.ylabel("Light Intensity")
            savename = "../../../bar_transient_" +  str(ix) + "_" + str(iy) + ".png"
            plt.savefig(savename,dpi=1200)
        plt.show()
    #return
    return np.mean(err_start), np.mean(err_start_max), np.mean(err_bar), MAE, EMD, np.mean(trans_nod), np.mean(trans_nod_cum), frac_start, MAE_ampl_base, MAE_ampl_direct, MAE_depth_base, MAE_depth_direct
