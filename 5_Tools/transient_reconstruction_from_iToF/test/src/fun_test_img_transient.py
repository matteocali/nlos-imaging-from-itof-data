...
def test_img(name,weights_path,attempt_name,Sname,P,freqs,fl_scale,fl_norm_perpixel,fil_dir,fil_den,fil_auto,fl_test_old=False,old_weights=None,Pold=3,fl_newhidden=True,dim_t=2000):
    ...
    for xx,name in enumerate(loadnames):
        ...
        if fl_test_old:
            ...
            err_so, err_som, err_bo, err_maeo, err_emdo, mean_pdfo, mean_cdfo, frac_starto, dir_amp_baseo, dir_amp_predo, dir_pos_baseo, dir_pos_predo = metrics_computation_transient_images(tr, x_g, xg_old, v_out_d_old, np.squeeze(v_in),freqs=freqs)
        ...
        err_s, err_sm, err_b, err_mae, err_emd, mean_pdf, mean_cdf, frac_start, dir_amp_base, dir_amp_pred, dir_pos_base, dir_pos_pred  = metrics_computation_transient_images(tr, x_g, x_out_g, v_out_d, v_in,freqs=freqs)
        ...
        if fl_auto:
            ...
            err_s, err_sm,  err_b, err_mae, err_emd, mean_pdf, mean_cdf, frac_start, dir_amp_base, dir_amp_pred, dir_pos_base, dir_pos_pred  = metrics_computation_transient_images(tr, x_g, x_dec, v_out_d, v_in, freqs=freqs)
