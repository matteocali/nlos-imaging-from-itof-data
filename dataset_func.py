...
from .transient_handler import (
    transient_loader,
    grid_transient_loader,
    compute_distance_map,
    phi,
    amp_phi_compute,
)
from .fermat_tools import (
    undistort_depthmap,
    compute_bin_center,
    prepare_fermat_data,
)
    ...
    for i in range(n_samples):
        ...
        for exp_val in exp_coeff:
            for sigma in sigma_blur:
                ...
                dgy_one = maximum(
                    convolve(y, f, "same"), convolve(y, np_flip(f), "same")
                )
        ...
        try:
            save_h5(
                ...
                data={
                    ...
                    "depth_map": swapaxes(gt["depth_map"], 0, 1),
                    "alpha_map": swapaxes(gt["alpha_map"], 0, 1),
                },
                fermat=False,
            )  # Save the file
        except KeyError:
...
def ampl_ratio_hists(dts_path: Path, out_path: Path) -> None:
    ...
    if not (
        ...
        for dts_file in tqdm(
            ...
            ampl_ratios.append(
                np_nanmax(amp_ratio) - np_nanmin(amp_ratio)
